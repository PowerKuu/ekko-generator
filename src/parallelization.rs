...
pub fn generate_batch_coords(start_index: usize, batch_size: usize, total_chunks: usize) -> Vec<(i32, i32)> {
    ...
    S: Fn(&ProtoChunk, i32, i32) + Send + Sync + Clone + std::panic::RefUnwindSafe,
{
    ...
    for (batch_index, (start_index, batch_size)) in batch_ranges.iter().enumerate() {
        ...
        if is_shutdown_requested() {
            println!("🛑 Shutdown requested, stopping at batch {}/{}", batch_index + 1, batch_ranges.len());
            break;
        }
        
        let batch_coords = if config.use_radius_generation {
            let actual_radius = config.radius.unwrap_or_else(|| {
                calculate_radius_for_chunks(total_chunks)
            });
            generate_radius_batch_coords(*start_index, *batch_size, config.center_x, config.center_z, actual_radius)
        } else {
        ...
        let batch_result = std::panic::catch_unwind(|| {
            process_batch_parallel(&batch_coords, Arc::clone(&generation_settings), chunk_callback.clone())
        });
        ...
        match batch_result {
            Ok(completed_successfully) => {
                ...
                let progress_percent = ((*start_index + *batch_size) as f64 / total_chunks as f64) * 100.0;
...
pub fn create_batch_ranges(start_index: usize, total_chunks: usize, batch_size: usize) -> Vec<(usize, usize)> {
...
pub fn process_chunks_simple<F>(chunk_coords: Vec<(i32, i32)>, config: &Config, dimension: Dimension, chunk_callback: F) 
where
    F: Fn(&ProtoChunk, i32, i32) + Send + Sync,
{
    let generation_settings = Arc::new(create_generation_settings(Seed(config.seed), &dimension));
