...
use pumpkin_world::{dimension::Dimension, generation::Seed, ProtoChunk};
use crate::height_map::{create_generation_settings, generate_surface_heights, get_chunk_proto};
use crate::config_loader::{load_config, Config};
use crate::parallelization::{
    calculate_radius_for_chunks, create_batch_ranges, 
    get_radius_stats, process_all_batches, process_chunks_simple, BatchStats
};
use crate::shutdown_handler::{get_current_chunk_index, is_shutdown_requested, setup_shutdown_handler, update_current_chunk_index};
use crate::storage::BlockStorage; // Import your BlockStorage
...
pub async fn run() {
    ...
    // Initialize BlockStorage with database connection
    let database_url = "postgresql://username:password@localhost/database_name"; // Get from config
    let batch_size = 100; // Adjust based on your needs - could also come from config
    
    let storage = match BlockStorage::new(database_url, batch_size).await {
        Ok(storage) => {
            println!("✅ Connected to database with batch size: {}", batch_size);
            Arc::new(storage)
        }
        Err(e) => {
    ...
    let total_chunks = if config.use_radius_generation {
        let radius = config.radius.unwrap_or_else(|| {
            calculate_radius_for_chunks(config.chunks_to_load)
        });
        ...
        println!("  - Diameter: {} chunks ({} blocks)", diameter, diameter * 16);
    ...
    println!("Generating {} chunks in batches of {} around center ({}, {})", 
             total_chunks - start_index, config.chunk_batch_size, center_x, center_z);
    ...
    let chunk_callback = {
        ...
        move |proto: &ProtoChunk, chunk_x: i32, chunk_z: i32| {
            ...
            let height_map_owned = proto.flat_surface_height_map.to_vec(); // Clone to owned Vec
            
            // Spawn async task for database operation
            tokio::spawn(async move {
                match storage_ref.queue_chunk(chunk_x, chunk_z, height_map_owned).await {
                    Ok(was_stored) => {
                        if was_stored {
                            println!("💾 Batch stored to database (triggered by chunk {}, {})", chunk_x, chunk_z);
                        }
                    }
                    Err(e) => {
                        eprintln!("❌ Failed to queue chunk ({}, {}): {}", chunk_x, chunk_z, e);
                    }
                }
            });
        }
    ...
    let progress_callback = |stats: BatchStats| {
        ...
        println!("Batch {}/{} completed - Progress: {:.2}% ({}/{}) | Batch time: {:?} | {:.1} chunks/sec | {:.2}ms/chunk | Overall: {:.1} chunks/sec", 
                 stats.batch_index, 
                 stats.total_batches, 
    ...
    match result {
        Ok(stats) => {
            if is_shutdown_requested() {
                ...
                println!("📊 Processed {} chunks before stopping", current_index - start_index);
                println!("💾 To resume, update your config chunks_start_index to: {}", current_index);
            } else {
                println!("✅ Generation completed successfully!");
                println!("Total time: {:?}", total_elapsed);
                println!("Average time per chunk: {:.2}ms", stats.average_ms_per_chunk);
                println!("Overall processing rate: {:.1} chunks/sec", stats.overall_chunks_per_sec);
            }
        }
        Err(failed_at_index) => {
            ...
            if chunks_completed > 0 {
                println!("Average time per chunk: {:.2}ms", 
                         total_elapsed.as_millis() as f64 / chunks_completed as f64);
            }
            println!("💾 To resume, update your config chunks_start_index to: {}", failed_at_index);
        }
    }
...
pub fn accuracy_test() {
    ...
    println!("Generated chunk with seed {} in dimension {:?}.", seed.0, dimension);
}
...
pub async fn parallel_test_with_storage() {
    ...
    let storage = Arc::new(BlockStorage::new(database_url, 50).await.expect("Failed to create storage"));
    storage.create_raw_table().await.expect("Failed to create table");
    ...
    process_chunks_simple(
        ...
        move |proto, chunk_x, chunk_z| {
            ...
            tokio::spawn(async move {
                if let Err(e) = storage_ref.queue_chunk(chunk_x, chunk_z, height_map_owned).await {
...
pub async fn radius_test_with_storage() {
    ...
    let chunk_coords = crate::parallelization::generate_radius_coords(center_x, center_z, radius);
    
    println!("Testing radius generation with {} chunks in radius {}", chunk_coords.len(), radius);
    let start_time = Instant::now();
    
    // Create storage for testing
    let database_url = "postgresql://username:password@localhost/database_name";
    let storage = Arc::new(BlockStorage::new(database_url, 25).await.expect("Failed to create storage"));
    storage.create_raw_table().await.expect("Failed to create table");
    ...
    process_chunks_simple(
        ...
        move |proto, chunk_x, chunk_z| {
            ...
            tokio::spawn(async move {
                if let Err(e) = storage_ref.queue_chunk(chunk_x, chunk_z, height_map.to_vec()).await {
                    eprintln!("Failed to queue chunk: {}", e);
                }
            });
        }
