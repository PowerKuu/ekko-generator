...
use pumpkin_world::{
    dimension::Dimension, generation::{chunk_noise::CHUNK_DIM, positions::chunk_pos::{start_block_x, start_block_z}, settings::gen_settings_from_dimension, Seed}, ProtoChunk, ProtoNoiseRouters
};
...
pub fn generate_surface_heights(proto: &mut ProtoChunk) {
    ...
    for cell_x in 0..horizontal_cells {
        ...
        for cell_z in 0..horizontal_cells {
            ...
            let adaptive_start = ((estimated_surface_y - search_range - min_y as i32) / vertical_cell_block_count as i32).max(start_cell as i32) as u16;
            let adaptive_end = ((estimated_surface_y + search_range - min_y as i32) / vertical_cell_block_count as i32).min(end_cell as i32) as u16;
                        ...
                        for local_z in 0..horizontal_cell_block_count {
                            ...
                            if !block_state.is_air() {
                                ...
                                if surface_samples.len() <= SAMPLES_FOR_CONVERGENCE {
                                    ...
                                    estimated_surface_y = surface_samples.iter().sum::<i32>() / surface_samples.len() as i32;
                                } else {
                                    // Weighted average favoring recent samples
                                    let weight = 0.1;
                                    estimated_surface_y = ((1.0 - weight) * estimated_surface_y as f32 + weight * block_y as f32) as i32;
                                }
                            }
                        }
                    }
