...
impl ZarrBlockStorage {
    ...
    pub async fn new(
        ...
        metadata.insert(
            ...
            Value::String("Minecraft chunk height maps generated by Ekko Generator".to_string()),
        );
        metadata.insert(
            "data_type".to_string(),
            Value::String("height_map".to_string()),
        );
        metadata.insert("units".to_string(), Value::String("blocks".to_string()));
        metadata.insert(
            "coordinate_system".to_string(),
            Value::String("minecraft_chunk_coordinates".to_string()),
        );
        metadata.insert(
            "coordinate_offset".to_string(),
            Value::Number(Number::from(SAFE_COORDINATE_OFFSET)),
        );
        metadata.insert(
            "zarr_region_size".to_string(),
            Value::Number(Number::from(zarr_region_size)),
        );
        metadata.insert(
            "chunk_dimension".to_string(),
            Value::Number(Number::from(CHUNK_DIM)),
        );
        metadata.insert(
        ...
        metadata.insert(
            ...
            Value::String("ekko-generator-0.1.0".to_string()),
        );
        ...
        if let Some(gen_meta) = generation_metadata {
            metadata.insert(
                "seed".to_string(),
                Value::Number(Number::from(gen_meta.seed)),
            );
            metadata.insert(
                "center_x".to_string(),
                Value::Number(Number::from(gen_meta.center_x)),
            );
            metadata.insert(
                "center_z".to_string(),
                Value::Number(Number::from(gen_meta.center_z)),
            );
            metadata.insert(
                "radius_start".to_string(),
                Value::Number(Number::from(gen_meta.radius_start)),
            );
            metadata.insert(
                "radius_end".to_string(),
                Value::Number(Number::from(gen_meta.radius_end)),
            );
            metadata.insert(
                "radius_circular".to_string(),
                Value::Bool(gen_meta.circular),
            );
        }
    ...
    async fn _flush_chunks(&self, chunks: HashMap<(i32, i32), Vec<i64>>) -> ZarrResult<()> {
        ...
        let mut zarr_region_groups: HashMap<(i32, i32), Vec<((i32, i32), Vec<i64>)>> =
            HashMap::new();
        ...
        for ((region_x, region_z), chunks) in zarr_region_groups {
            ...
            self.array.store_array_subset(&subset, bytes).map_err(|e| {
                format!(
                    "Failed to store zarr region ({}, {}) at range {}..{}, {}..{}: {}",
                    region_x,
                    region_z,
                    region_x_start,
                    region_x_start + zarr_region_blocks as u64,
                    region_z_start,
                    region_z_start + zarr_region_blocks as u64,
                    e
                )
            })?;
        }
